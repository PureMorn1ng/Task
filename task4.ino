...
bool isWolfButtonOn = 0;
bool currentWolfVal;
bool previousWolfVal;
long unsigned int tWolf;
...
void checkTrafficLigths() {
  ...
  if (millis() - tlast > 15000 || isWolfButtonOn == 1) {
    GreenFlag = 1;
  }
...
void WolfButton() {
  int buttonWolfVal = analogRead(WolfButtonPin);
  if (buttonWolfVal > 0) {
    currentWolfVal = 1;
    if (currentWolfVal > previousWolfVal) {   // отлавливаем переход к 1
      isWolfButtonOn = !isWolfButtonOn;
      if (isWolfButtonOn == 1)
        tWolf = millis();
      else {
        St_last = 3;
...
else  currentWolfVal = 0;
previousWolfVal = currentWolfVal;
}
